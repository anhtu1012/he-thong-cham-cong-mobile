  ...
  try {
    ...
    if (contractResponse.status === 200) {
      ...
      const activeContract = Array.isArray(contractData)
        ? contractData.find((contract) => contract.status === "ACTIVE") ||
          contractData[0]
        : contractData;
  ...
  try {
    ...
    if (!contractData) {
      ...
      finalContractData = {
        ...
        contractEndTime: contract.endTime
        ...
        [
          {
            ...
            onPress: () => {
              ...
              Sharing.shareAsync(newFileUri).catch(() => {
                Alert.alert(
                  "Thông báo",
                  "Không thể mở file. Vui lòng kiểm tra trong thư mục Documents."
                );
              });
            },
          },
    ...
    Alert.alert("Lỗi khi tải file", errorMessage, [
      {
        text: "Thử lại",
